// Redesign using a different joint instead of the mortisse joint.
...
module PegHoles(height, count)
{
    ...
    translate(
        [
            -1,
            TabDepth/2, 
            GetPegSpacing(height, count) * (i+1) + PegRadius + PegDiameter * i
        ])
    rotate([0, 90, 0])
        cylinder(
            h = TwoByFourDims[0] + 2,
    ...
    difference()
    {
        ...
        translate(
            [
                ...
                height - GetTabHeight(TwoByFourDims[1])
            ])
            cube(
                [TabWidth + 0.1, TabDepth + 0.1, GetTabHeight(TwoByFourDims[1]) + 0.1],
                center = false);
        echo("top rail notch Z", height - GetTabHeight(TwoByFourDims[1]));
        ...
        translate(
            ...
            cube(
                [TabWidth + 0.1, TabDepth + 0.1, GetTabHeight(TwoByFourDims[1])],
                center = false);
        echo("bottom rail notch Z", FootHeight);
...
module SideLegFront(height)
{
    ...
    difference()
    {
        ...
        translate(
            ...
            cube(
                [TabDepth + 1, TabWidth + 0.1, GetTabHeight(TwoByFourDims[1])],
                center = false); 
...
module SideBottomRail(width)
{
    ...
    difference()
    {
        ...
        translate([TabWidth, -0.1, -0.1])
            cube([TabWidth + 0.1, TabDepth + 0.1, TwoByFourDims[1] + 0.2]);
        translate([0, width + TabDepth * 2, 0])
            mirror([0, 1, 0])
            translate([TabWidth, -0.1, -0.1])
                cube([TabWidth + 0.1, TabDepth + 0.1, TwoByFourDims[1] + 0.2]);
...
module FaceBottomRail(length, section_count)
{
    ...
    difference()
    {
        ...
        translate([TabWidth, -0.1, -0.1])
            cube([TabWidth + 0.1, TabDepth + 0.1, TwoByFourDims[1] + 0.2]);        
        NegativeTab(TwoByFourDims[1]);
        translate([0, length + TabDepth * 2, 0])
            mirror([0, 1, 0])
                translate([TabWidth, -0.1, -0.1])
                    cube([TabWidth + 0.1, TabDepth + 0.1, TwoByFourDims[1] + 0.2]);
